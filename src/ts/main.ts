...
// --- Product Slider ---
type Product = { title: string; price: string; image: string; oldPrice?: string; };
...
function renderDots() {
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (i === currentSlide) dot.classList.add("bg-primary");
...
function goToSlide(idx: number) {
  currentSlide = idx;
  const cardWidth = slider.querySelector("div")?.clientWidth || 220;
...
// --- Testimonials Slider ---
type Testimonial = { name: string; role: string; text: string; image: string; };
...
window.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll<HTMLElement>(".animate-fade-in").forEach((el, i) => {
    el.style.opacity = "0";
    setTimeout(() => {
